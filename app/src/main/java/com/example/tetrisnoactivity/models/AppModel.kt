...
class AppModel {
        ...
        if (isGameActive()) {
            ...
            if (!moveValid(coordinate as Point, frameNumber)) {
                ...
                if (Motions.DOWN.name == action) {
                    ...
                    assessField()
                    generateNextBlock()
                    if (!blockAdditionPosible()) {
                        currentState = Statusus.OVER.name;
                        currentBlock = null;
                        resetField(false);
                    }
                }
    ...
    private fun blockAdditionPosible(): Boolean {
        if (!moveValid(
                currentBlock?.position as Point,
    ...
    fun startGame(){
        if (!isGameActive()) {
            currentState = Statusus.ACTIVE.name
            generateNextBlock()
        }
    }
    fun restartGame() {
    ...
    fun endGame() {
        score = 0
        currentState = AppModel.Statusus.OVER.name
    }
    private fun resetModel() {
        resetField(false)
        currentState  = Statusus.AWAITING_START.name
        score = 0
    }
